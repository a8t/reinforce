...
big_ass_hash[:response][:results].each do |each_article|
  each_article[:views] = 0
end
...
def display_views(hash)
  hash[:response][:results].each do |each_article|
    puts "#{each_article[:views]} views for #{each_article[:webTitle]}."
  end
end


6.times do read_article(big_ass_hash) end

display_views(big_ass_hash)
